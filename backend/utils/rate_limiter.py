...
class DistributedRateLimiter:
    ...
    async def initialize(self):
        ...
        if redis_url and redis_token:
            try:
                ...
                print("[RateLimiter] ✅ Connected to Upstash Redis (Distributed Mode)")
                self._initialized = True
                return True
            except Exception as e:
                print(f"[RateLimiter] ⚠️ Redis connection failed: {e}")
                print("[RateLimiter] 📋 Falling back to local mode")
                self.redis = None
        else:
            print("[RateLimiter] 📋 No Redis configured, using local mode")
    ...
    async def acquire(
        ...
        print(f"[RateLimiter] ⚠️ Timeout waiting for quota, proceeding anyway")
        return True, region
    ...
    def record_failure(self, region: str, error: str = ""):
        ...
        if self._failure_counts[region] >= self.config.failure_threshold:
            ...
            print(f"[RateLimiter] 🔴 Circuit OPEN for region {region}: {error}")
