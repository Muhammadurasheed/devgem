    ...
    async def _handle_deploy_to_cloudrun(
        ...
        try:
            ...
            if domain_result['success']:
                ...
                deploy_result['url'] = f"https://{domain_result['domain']}" # Update to real URL
...
Showing last {min(20, len(logs))} entries (total: {len(logs)})
    ...
    def load_state(self, state: Dict[str, Any]):
        ...
        history_data = state.get('chat_history', [])
            ...
            for h in history_data:
                ...
                for p in h.get('parts', []):
                    if 'text' in p:
                        ...
                        # Use from_dict to avoid direct class usage issues
                ...
                if parts:
                    history_objects.append(Content(role=h['role'], parts=parts))
        else:
            # Gemini API (google-generativeai) uses simpler dicts or different objects
            # For simplicity, we can try using the dict format which the SDK often accepts
            # OR map to google.generativeai.types
            import google.generativeai as genai
            # The genai SDK is flexible with dicts for history
            history_objects = history_data

        # Initialize chat session with restored history
        try:
            self.chat_session = self.model.start_chat(history=history_objects)
            print(f"[Orchestrator] ✅ Restored session with {len(history_objects)} messages")
        except Exception as e:
            print(f"[Orchestrator] ⚠️ Failed to restore history: {e}")
